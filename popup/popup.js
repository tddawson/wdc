(async function () {
  ...
  const { features } = await chrome.runtime.sendMessage({ type: "GET_FEATURES" });
  ...
  for (const feature of features) {
    ...
    btn.className = "feature-btn" + (activeFeature === feature.id ? " active" : "");
  ...
  async function handleClick(feature, btn) {
    ...
    list.querySelectorAll(".feature-btn").forEach((b) => b.classList.remove("active"));
